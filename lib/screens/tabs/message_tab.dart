...
class _MessageTabState extends State<MessageTab> {
  ...
  @override
  void dispose() {
    _inputCtrl.dispose(); _scrollCtrl.dispose(); _searchCtrl.dispose();
    super.dispose();
  }

  @override
  Widget build(BuildContext context) {
    return _activeRoom != null ? _buildChatRoom() : _buildRoomList();
  }
  ...
  Widget _buildRoomTile(_ChatRoom room) {
    return InkWell(
      onTap: () => setState(() { _activeRoom = room; _messages = List.from(_initMessages); }),
      child: Container(
      ...
      child: Row(
        children: [
          IconButton(
            ...
            onPressed: () => setState(() { _activeRoom = null; _showSearch = false; _showAttachPanel = false; }),
          ),
          Container(
            width: 36, height: 36,
            decoration: BoxDecoration(color: AppColors.bg, shape: BoxShape.circle, border: Border.all(color: AppColors.border)),
          ...
          Expanded(child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text(_activeRoom!.name, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w700)),
          ...
          child: Column(
            ...
            children: [
              ...
              ListTile(
                ...
                onTap: () { Navigator.pop(context); setState(() => _activeRoom = null); },
              ),
            ],
          ),
        ),
